...
1. **The NSD permissions spike is a gate on the whole product premise, and must run before slice 1.** The PRD's "no runtime permissions at all" claim is very probably correct — `NEARBY_WIFI_DEVICES` gates physical-proximity radio APIs, not same-subnet mDNS — but it is unverified on Android 13–16 and it is doing marketing work in the PRD's Solution section. Half an hour, two physical devices, a manifest with only `INTERNET` and `ACCESS_NETWORK_STATE`. If it fails, the hedge is `NEARBY_WIFI_DEVICES` with `neverForLocation` — one prompt, survivable, but the PRD's framing changes.

2. **Local Network Protection may put this app behind a runtime permission on future Android.** Android 16 introduced work gating local-subnet access — mDNS *and* direct RFC1918 socket connections — behind a new permission, believed to have shipped opt-in with enforcement intended later. Confirm current status before finalising the no-permissions claim. This is why `targetSdk` is held at 35.
