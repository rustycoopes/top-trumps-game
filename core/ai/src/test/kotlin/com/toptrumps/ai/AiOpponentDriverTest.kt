...
private fun fakeSession(): MatchSession = object : MatchSession {
    ...
    override val completedMatch = MutableStateFlow<RecordedMatch?>(null)
...
class AiOpponentDriverTest {
    ...
    fun `the AI resolves the round on its own when it is the chooser`() = runTest(UnconfinedTestDispatcher()) {
        ...
        for (seed in 0 until 50) {
            ...
            val host = HostMatchSession(testDeck(), MatchConfig("test-deck"), Random(seed), hostTransport, backgroundScope, "test-token", "Opponent")
            val guest = GuestMatchSession(guestTransport, backgroundScope, "test-token", "Opponent")
